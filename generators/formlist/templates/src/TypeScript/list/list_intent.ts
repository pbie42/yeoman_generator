...
export function intent({ DOM, HTTP, new<%= itemNameU %>, edit<%= itemNameU %> }:ListSources):ListIntent {
  ...
  const <%= itemNameL %>EditSuccess:Stream<Array<State>> = queries.responses.edit<%= itemNameU %>

  const actions:Stream<Function> = queries.actions
  const add<%= itemNameU %>:Stream<State | {}> = Stream.merge(sample(new<%= itemNameU %>, <%= itemNameL %>SaveSuccess), loaded<%= itemNameU %>)
