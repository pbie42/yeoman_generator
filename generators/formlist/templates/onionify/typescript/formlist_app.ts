...
export default function app({DOM, HTTP, onion }:Sources):Sinks {
  ...
  const form:FormSinks = isolate(<%= formName %>, { onion: formListLens })({ DOM, HTTP, onion }, editsProxy)
  const list:ListSinks = isolate(<%= listName %>, { onion: formListLens })({ DOM, HTTP, onion }, form.new<%= itemNameU %>, form.edit<%= itemNameU %>)

  const reducer:Stream<Reducer> = Stream.merge(initReducer, form.onion, list.onion)
